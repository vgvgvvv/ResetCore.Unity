...
namespace ResetCore.SAsset
{
    ...
    public class ABLoader : IStorable
    {
        ...
        /// 加载上下文
        /// </summary>
        public ABLoadContext _loadContext { get; private set; }
        ...
        private void RequestLoadBundle(bool immediately)
        {
            ...
            {
                _loadContext.waitingLoaderQueue.Enqueue(this);
                LoadWaitingLoader();
            }
        }
        ...
        {
            while (_loadContext.waitingLoaderQueue.Count > 0 && _loadContext.loadingCount < _loadContext.LOADING_LIMIT)
            {
                ABLoader loader = _loadContext.waitingLoaderQueue.Dequeue();
        ...
        private void OnComplete()
        {
            if (abObject == null)
            {
                ...
                _loadContext.loadedABDict[abObject.abName] = abObject;
            }
        }
