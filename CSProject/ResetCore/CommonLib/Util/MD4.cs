...
namespace ResetCore.Util
{
    public class MD4 : HashAlgorithm
    {
        ...
        public static MD4 Get()
        {
            var result = new MD4();
            result._x = new uint[16];
            result.Initialize();
            return result;
        }
        ...
        protected override byte[] HashFinal()
        {
            try
            {
                ...
                return new[] { _a, _b, _c, _d }.SelectMany(Bytes).Cast<byte>().ToArray();
            }
            finally
            {
                Initialize();
        ...
        private static IEnumerable<uint> Bytes(byte[] bytes, int offset, int length)
        {
        ...
        private IEnumerable<uint> Bytes(uint word)
        {
        ...
        private IEnumerable<uint> Repeat(byte value, int count)
        {
        ...
        private IEnumerable<uint> Padding()
        {
            return Repeat(128, 1)
    ...
    {
        public static int Compute(string nameSpace, string typeName)
        {
            string toBeHashed = "s\0\0\0" + nameSpace + typeName;

            using (HashAlgorithm hash = MD4.Get())
            {
